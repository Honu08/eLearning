...
//time picker function for calendars
...
function getUsersOptions(username){
	performAjax({
		"task": "get_users",
		"username": username
	}, function (data) {
		var json = JSON.parse (data);
		console.info(data);
		var select_options = '<option disabled selected=true>Choose a user</option>';
		if (json.constructor === Array) {
			for (var i = 0; i < json.length; i++) {
				select_options += '<option>' + json[i].username + '</option>';
			}
		} else {
			select_options += '<option>' + json.name+" "+json.lastName + '</option>';
		}
		$("#existing-registered-users").html (select_options);
	});
}

$("#existing-registered-users").change(function(){
	performAjax({
		"task":"get_roles",
		"user": $(this).val()
	},function(data){
		...
		if(json[0].role == "admin"){
			...
			$("#input-checkbox-admin").prop ("checked", false);
		}
	});
});
...
$("#update_role").click(function(){
	var valid = true;
	var checkbox = "";
	if($("#existing-registered-users").val() === null){
		$("#user-role-alert").show("fast");
			setTimeout(function() {
	...
	if(valid){
		...
		bootbox.confirm({
			...
			message: "<strong>&nbsp;Promote: </strong> " + $("#existing-registered-users").val() ,
			buttons: {
				'cancel': {
					label: 'No',
			...
			callback: function(result) {
				if (result) {
					performAjax({
						"task":"update_role",
						"user": $("#existing-registered-users").val(),
						"role": checkbox
					},function(data){
						var json = JSON.parse(data);
						if(json.success){
